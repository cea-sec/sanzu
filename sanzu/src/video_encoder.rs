...
impl Encoder for EncoderFFmpeg {
    ...
    fn encode_image(
        ...
        let pixel_format = unsafe { (*self.frame.as_ptr()).format };

        match pixel_format {
            x if x == AVPixelFormat::AV_PIX_FMT_YUV420P as i32 => {
                // yuv420
                let (y_lane, u_lane, v_lane) = unsafe {
                    let y_lane = (*self.frame.as_ptr()).linesize[0] as u32;
                    let u_lane = (*self.frame.as_ptr()).linesize[1] as u32;
                    let v_lane = (*self.frame.as_ptr()).linesize[2] as u32;
            ...
            x if x == AVPixelFormat::AV_PIX_FMT_NV12 as i32 => {
                ...
                let (y_lane, uv_lane) = unsafe {
                    let y_lane = (*self.frame.as_ptr()).linesize[0] as u32;
                    let uv_lane = (*self.frame.as_ptr()).linesize[1] as u32;
                    (y_lane, uv_lane)
                };
            ...
            x if x == AVPixelFormat::AV_PIX_FMT_RGB0 as i32 => {
                ...
                let plane_bpl = unsafe { (*self.frame.as_ptr()).linesize[0] } as usize;
